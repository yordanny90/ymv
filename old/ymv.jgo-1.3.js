 ...
 */
$jgo=(function($,$jgo){
	$jgo=function(opts,cfg,save){
		if(!opts || (typeof opts!='string' && typeof opts!='object') || (typeof opts=='object' && !opts.ambit)) throw new Error('Ambit missing');
		if(typeof $jgo_ambit!='object')
			$jgo_ambit={};
		if(arguments.length==1 && typeof opts=='string' && $jgo_ambit[opts])
			return $jgo_ambit[opts];
		if(this.constructor!=$jgo)
			return new $jgo(opts,cfg,save);
		// Configuracion Ajax
		...
		// Almacenar
		if(save)
			$jgo_ambit[typeof save=='string'?save:this.opts.ambit]=this;
	};
	$jgo.default_opts={
		dom:'jgo',
		ambit:'',
	...
	$jgo.fn.setJSON=function(json){
		...
		$.each($jgo.jsonNodes,function(attr,fn){
	...
	$jgo.fn.setXML=function(xml){
		...
		$.each($jgo.xmlNodes,function(attr,fn){
			if(typeof fn.fn=='function')
	...
	$jgo.fn.setTEXT=function(text){
		...
		$.each($jgo.textNodes,function(attr,fn){
			if(typeof fn.fn=='function')
	...
	$jgo.fn.cloneFor=function(root,save){
		return $jgo($joq(this.opts).select().update({root:root}).val(),this.cfg,save);
	};
	return $jgo;
})(jQuery);
